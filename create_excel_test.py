...
for f in f_name:
    ...
    masked_img = cv2.bitwise_and(img,img,mask = mask)               # マスク画像合成
    brightness = cv2.mean(img)                                      # 平均輝度の取得
    mask_brightness = cv2.mean(masked_img)
    ...
    mask_contours,mask_hierarchy = cv2.findContours(mask_thresh, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    mask_max_id, mask_max_area = -1, 0
    for j in range(len(mask_contours)):
        #print('ID', j, 'Area', cv2.contourArea(mask_contours[j]))
        mask_cnt = mask_contours[j]
        mask_area = cv2.contourArea(mask_cnt)
        # 最大輪郭を見つける
        if mask_area > mask_max_area:
            mask_max_area = mask_area
            mask_max_id = j
